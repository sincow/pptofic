<div class="content p-2 pt-10">
	<div class="row mb-2">
   	<div class="col-lg-8">
      	<h4 class="mb-0">Origenes Especificos de Ingresos</h4>
   	</div>
   	<div class="col-lg-4">
   		<nav class="mb-0" aria-label="breadcrumb">
      		<ol class="breadcrumb mb-0 float-sm-end">
         		<li class="breadcrumb-item"><a href="./">Dashboard</a></li>
         		<li class="breadcrumb-item active">Origenes Especificos de Ingresos</li>
      		</ol>
      	</nav>
   	</div>
	</div>
	<form role="form" class="origingresoFormList" method="post" action="origingresoDetails">
   	<div class="mb-3">
   		<div id="OrigIngreso" data-list='{"valueNames":["id", "name", "initials", "status"],"page":15,"pagination":true}'>
      		<div class="mb-0">
         		<div class="d-flex flex-wrap gap-3">
         			<div class="search-box">
							<form class="position-relative" data-bs-toggle="search" data-bs-display="static">
								<input class="form-control search-input search" id="searchOrigIngreso" name="searchOrigIngreso" type="search" placeholder="Buscar Origenes de Ingresos" aria-label="Search" />
						...
						<?php
							$permiAdd = "";
							$permiExp = "";
							$opcion = array_search("origingresoadd", array_column($_SESSION['permissionssin'], 'OpcLink'));
							if ($opcion !== NULL && $opcion !== FALSE) {
								$permi = $_SESSION['permissionssin'][$opcion]["UsuPermi"];
								if ($permi != 0) {
									$permiAdd = '<a class="btn btn-primary" href="?action=create"><span class="fas fa-plus me-2"></span>Agregar Origen de Ingreso</a>';
									$permiAdd = '<a class="btn btn-primary" id="btnaddOrigIngreso" href="origingresoadd"><span class="fas fa-plus me-2"></span>Adicionar Origen de Ingreso</a>';
									$permiAdd = '<a class="btn btn-primary" id="btnaddOrigIngreso"><span class="fas fa-plus me-2"></span>Agregar Origen de Ingreso</a>';
								}
							}
							$opcion = array_search("origingresoexport", array_column($_SESSION['permissionssin'], 'OpcLink'));
							if ($opcion !== NULL && $opcion !== FALSE) {
								$permi = $_SESSION['permissionssin'][$opcion]["UsuPermi"];
								if ($permi != 0) {
                     ...
                     $permiMod = "";
                     $permiDel = "";	
                     $opcion = array_search("origingresoedit", array_column($_SESSION['permissionssin'], 'OpcLink'));
                     if ($opcion !== NULL && $opcion !== FALSE) {
                        $permi = $_SESSION['permissionssin'][$opcion]["UsuPermi"];
                        if ($permi != 0) {
                           $permiModsw = "1";
                     ...
                     $opcion = array_search("origingresodelete", array_column($_SESSION['permissionssin'], 'OpcLink'));
                     if ($opcion !== NULL && $opcion !== FALSE) {
                        $permi = $_SESSION['permissionssin'][$opcion]["UsuPermi"];
                        if ($permi != 0) {
                           $permiDelsw = "1";
				...
				<div class="mx-n4 px-4 mx-lg-0 px-lg-2 bg-white border-top border-bottom border-200 position-relative top-1">
					<div class="table-responsive scrollbar mx-n1 px-1">
						<form role="form" class="mb-1 especieFormEdit" method="post" action="especiemodify">
							<table class="table table-striped mb-0 origIngresoTable" style="font-size: 12px;" id="origingresoTable">
								...
								<tbody class="list" id="origingreso-table-body">
                           <tr>
	...
	<?php
		include "origingresoedit.php";
		include "origingresoadd.php";
		include APP_PATH.'/views/layouts/footer.php';
	?>
</div>
<script src="app/js/presupuesto/origingreso.js"></script>