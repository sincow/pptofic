<div class="content p-2 pt-10">
	<div class="row mb-2">
   	<div class="col-lg-8">
      	<h4 class="mb-0">Codigos Presupuestales de Ingresos</h4>
   	</div>
   	<div class="col-lg-4">
   		<nav class="mb-0" aria-label="breadcrumb">
      		<ol class="breadcrumb mb-0 float-sm-end">
         		<li class="breadcrumb-item"><a href="./">Dashboard</a></li>
         		<li class="breadcrumb-item active">Codigos Presupuestales de Ingresos</li>
      		</ol>
      	</nav>
   	</div>
	</div>
	<form role="form" class="rubroingresoFormList" method="post" action="rubroingresodetails">
   	<div class="mb-3">
   		<div id="RubroIngreso" data-list='{"valueNames":["id", "name", "initials", "status"],"page":15,"pagination":true}'>
      		<div class="mb-0">
         		<div class="d-flex flex-wrap gap-3">
         			<div class="search-box">
							<form class="position-relative" data-bs-toggle="search" data-bs-display="static">
								<input class="form-control search-input search" id="searchRubroIngreso" name="searchRubroIngreso" type="search" placeholder="Buscar Rubros de Ingreso" aria-label="Search" />
						...
						<?php
							$permiAdd = "";
							$permiExp = "";
							$opcion = array_search("rubroingresoadd", array_column($_SESSION['permissionssin'], 'OpcLink'));
							if ($opcion !== NULL && $opcion !== FALSE) {
								$permi = $_SESSION['permissionssin'][$opcion]["UsuPermi"];
								if ($permi != 0) {
									$permiAdd = '<a class="btn btn-primary" id="btnaddRubroIngreso"><span class="fas fa-plus me-2"></span>Agregar Rubro de Ingreso</a>';
								}
							}
							$opcion = array_search("rubroingresoexport", array_column($_SESSION['permissionssin'], 'OpcLink'));
							if ($opcion !== NULL && $opcion !== FALSE) {
								$permi = $_SESSION['permissionssin'][$opcion]["UsuPermi"];
								if ($permi != 0) {
                     ...
                     $permiMod = "";
                     $permiDel = "";
                     $opcion = array_search("rubroingresoedit", array_column($_SESSION['permissionssin'], 'OpcLink'));
                     if ($opcion !== NULL && $opcion !== FALSE) {
                        $permi = $_SESSION['permissionssin'][$opcion]["UsuPermi"];
                        if ($permi != 0) {
                           $permiModsw = "1";
                     ...
                     $opcion = array_search("rubroingresodelete", array_column($_SESSION['permissionssin'], 'OpcLink'));
                     if ($opcion !== NULL && $opcion !== FALSE) {
                        $permi = $_SESSION['permissionssin'][$opcion]["UsuPermi"];
                        if ($permi != 0) {
                           $permiDelsw = "1";
				...
				<div class="mx-n4 px-4 mx-lg-0 px-lg-2 bg-white border-top border-bottom border-200 position-relative top-1">
					<div class="table-responsive scrollbar mx-n1 px-1">
						<form role="form" class="mb-1 rubroingresoFormEdit" method="post" action="rubroingresomodify">
							<table class="table table-striped mb-0 rubroingresoTable" style="font-size: 12px;" id="rubroingresoTable">
								...
								<thead style="background-color: var(--phoenix-body-bg); backdrop-filter: blur(8px); opacity: 0.98;">
									<tr>
										...
										<th class="sort white-space-nowrap align-middle ps-2" scope="col" data-sort="status" style="width:10%;">Movimiento</th>
								...
								<tbody class="list" id="rubroingreso-table-body">
                           <tr>
	...
	<?php
		include "rubroingresoedit.php";
		include "rubroingresoadd.php";
		include APP_PATH.'/views/layouts/footer.php';
	?>
</div>
<script src="app/js/presupuesto/rubroingreso.js"></script>