<div class="content p-2 pt-10">
	...
	<form role="form" class="dependenciaFormList" method="post" action="dependenciadetails">
   	<div class="mb-3">
   		<div id="Dependencia" data-list='{"valueNames":["id", "name", "initials", "status"],"page":15,"pagination":true}'>
      		<div class="mb-0">
         		<div class="d-flex flex-wrap gap-3">
         			<div class="search-box">
							<form class="position-relative" data-bs-toggle="search" data-bs-display="static">
								<input class="form-control search-input search" id="searchDependencia" name="searchDependencia" type="search" placeholder="Buscar Dependencias" aria-label="Search" />
						...
						<?php
							$permiAdd = "";
							$permiExp = "";
							$opcion = array_search("dependenciaadd", array_column($_SESSION['permissionssin'], 'OpcLink'));
							if ($opcion !== NULL && $opcion !== FALSE) {
								$permi = $_SESSION['permissionssin'][$opcion]["UsuPermi"];
								if ($permi != 0) {
									$permiAdd = '<a class="btn btn-primary" id="btnaddDependencia"><span class="fas fa-plus me-2"></span>Agregar Dependencia</a>';
								}
							}
							$opcion = array_search("dependenciaexport", array_column($_SESSION['permissionssin'], 'OpcLink'));
							if ($opcion !== NULL && $opcion !== FALSE) {
								$permi = $_SESSION['permissionssin'][$opcion]["UsuPermi"];
								if ($permi != 0) {
                     ...
                     $permiMod = "";
                     $permiDel = "";
                     $opcion = array_search("dependenciaedit", array_column($_SESSION['permissionssin'], 'OpcLink'));
                     if ($opcion !== NULL && $opcion !== FALSE) {
                        $permi = $_SESSION['permissionssin'][$opcion]["UsuPermi"];
                        if ($permi != 0) {
                           $permiModsw = "1";
                     ...
                     $opcion = array_search("dependenciadelete", array_column($_SESSION['permissionssin'], 'OpcLink'));
                     if ($opcion !== NULL && $opcion !== FALSE) {
                        $permi = $_SESSION['permissionssin'][$opcion]["UsuPermi"];
                        if ($permi != 0) {
                           $permiDelsw = "1";
				...
				<div class="mx-n4 px-4 mx-lg-0 px-lg-2 bg-white border-top border-bottom border-200 position-relative top-1">
					<div class="table-responsive scrollbar mx-n1 px-1">
						<form role="form" class="mb-1 dependenciaFormEdit" method="post" action="dependenciamodify">
							<table class="table table-striped mb-0 dependenciaTable" style="font-size: 12px;" id="dependenciaTable">
								...
								<thead style="background-color: var(--phoenix-body-bg); backdrop-filter: blur(8px); opacity: 0.98;">
									<tr>
										...
										<th class="sort white-space-nowrap align-middle ps-2" scope="col" data-sort="initials" style="width: 10%;">Iniciales</th>
										<th class="sort white-space-nowrap align-middle ps-2" scope="col" data-sort="centrocto" style="width:10%;">Cen. Costo</th>
								...
								<tbody class="list" id="dependencia-table-body">
                           <tr>
	...
	<?php
		include "dependenciaedit.php";
		include "dependenciaadd.php";
		include APP_PATH.'/views/layouts/footer.php';
	?>
</div>
<script src="app/js/presupuesto/dependencia.js"></script>