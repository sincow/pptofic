<div class="content p-2 pt-10">
	...
	<form role="form" class="tipodocumentoFormList" method="post" action="tipodocumentodetails">
   	<div class="mb-3">
   		<div id="TipoDocumento" data-list='{"valueNames":["id", "name", "initials", "status"],"page":15,"pagination":true}'>
      		<div class="mb-0">
         		<div class="d-flex flex-wrap gap-3">
         			<div class="search-box">
							<form class="position-relative" data-bs-toggle="search" data-bs-display="static">
								<input class="form-control search-input search" id="searchTipoDocumento" name="searchTipoDocumento" type="search" placeholder="Buscar Tipos de Documento" aria-label="Search" />
						...
						<?php
							$permiAdd = "";
							$permiExp = "";
							$opcion = array_search("tipodocumentoadd", array_column($_SESSION['permissionssin'], 'OpcLink'));
							if ($opcion !== NULL && $opcion !== FALSE) {
								$permi = $_SESSION['permissionssin'][$opcion]["UsuPermi"];
								if ($permi != 0) {
									$permiAdd = '<a class="btn btn-primary" id="btnaddTipoDocumento"><span class="fas fa-plus me-2"></span>Agregar Tipo de Documento</a>';
								}
							}
							$opcion = array_search("tipodocumentoexport", array_column($_SESSION['permissionssin'], 'OpcLink'));
							if ($opcion !== NULL && $opcion !== FALSE) {
								$permi = $_SESSION['permissionssin'][$opcion]["UsuPermi"];
								if ($permi != 0) {
                     ...
                     $permiMod = "";
                     $permiDel = "";
                     $opcion = array_search("tipodocumentoedit", array_column($_SESSION['permissionssin'], 'OpcLink'));
                     if ($opcion !== NULL && $opcion !== FALSE) {
                        $permi = $_SESSION['permissionssin'][$opcion]["UsuPermi"];
                        if ($permi != 0) {
                           $permiModsw = "1";
                     ...
                     $opcion = array_search("tipodocumentodelete", array_column($_SESSION['permissionssin'], 'OpcLink'));
                     if ($opcion !== NULL && $opcion !== FALSE) {
                        $permi = $_SESSION['permissionssin'][$opcion]["UsuPermi"];
                        if ($permi != 0) {
                           $permiDelsw = "1";
				...
				<div class="mx-n4 px-4 mx-lg-0 px-lg-2 bg-white border-top border-bottom border-200 position-relative top-1">
					<div class="table-responsive scrollbar mx-n1 px-1">
						<form role="form" class="mb-1 tipodocumentoFormEdit" method="post" action="tipodocumentomodify">
							<table class="table table-striped mb-0 tipodocumentoTable" style="font-size: 12px;" id="tipodocumentoTable">
								...
								<tbody class="list" id="tipodocumento-table-body">
                           <tr>
	...
	<?php
		include "tipodocumentoedit.php";
		include "tipodocumentoadd.php";
		include APP_PATH.'/views/layouts/footer.php';
	?>
</div>
<script src="app/js/presupuesto/tipodocumento.js"></script>