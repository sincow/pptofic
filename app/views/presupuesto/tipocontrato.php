<div class="content p-2 pt-10">
	<div class="row mb-2">
   	<div class="col-lg-8">
      	<h4 class="mb-0">Tipos de Contrato</h4>
   	</div>
   	<div class="col-lg-4">
	...
	<form role="form" class="tipocontratoFormList" method="post" action="tipocontratodetails">
   	<div class="mb-3">
   		<div id="TipoContrato" data-list='{"valueNames":["id", "name", "initials", "status"],"page":15,"pagination":true}'>
      		<div class="mb-0">
         		<div class="d-flex flex-wrap gap-3">
         			<div class="search-box">
							<form class="position-relative" data-bs-toggle="search" data-bs-display="static">
								<input class="form-control search-input search" id="searchTipoContrato" name="searchTipoContrato" type="search" placeholder="Buscar Tipos de Contrato" aria-label="Search" />
						...
						<?php
							$permiAdd = "";
							$permiExp = "";
							$opcion = array_search("tipocontratoadd", array_column($_SESSION['permissionssin'], 'OpcLink'));
							if ($opcion !== NULL && $opcion !== FALSE) {
								$permi = $_SESSION['permissionssin'][$opcion]["UsuPermi"];
								if ($permi != 0) {
									$permiAdd = '<a class="btn btn-primary" id="btnaddTipoContrato"><span class="fas fa-plus me-2"></span>Agregar Tipo de Contrato</a>';
								}
							}
							$opcion = array_search("tipocontratoexport", array_column($_SESSION['permissionssin'], 'OpcLink'));
							if ($opcion !== NULL && $opcion !== FALSE) {
								$permi = $_SESSION['permissionssin'][$opcion]["UsuPermi"];
								if ($permi != 0) {
                     ...
                     $permiMod = "";
                     $permiDel = "";
                     $opcion = array_search("tipocontratoedit", array_column($_SESSION['permissionssin'], 'OpcLink'));
                     if ($opcion !== NULL && $opcion !== FALSE) {
                        $permi = $_SESSION['permissionssin'][$opcion]["UsuPermi"];
                        if ($permi != 0) {
                           $permiModsw = "1";
                     ...
                     $opcion = array_search("tipocontratodelete", array_column($_SESSION['permissionssin'], 'OpcLink'));
                     if ($opcion !== NULL && $opcion !== FALSE) {
                        $permi = $_SESSION['permissionssin'][$opcion]["UsuPermi"];
                        if ($permi != 0) {
                           $permiDelsw = "1";
				...
				<div class="mx-n4 px-4 mx-lg-0 px-lg-2 bg-white border-top border-bottom border-200 position-relative top-1">
					<div class="table-responsive scrollbar mx-n1 px-1">
						<form role="form" class="mb-1 tipocontratoFormEdit" method="post" action="tipocontratomodify">
							<table class="table table-striped mb-0 tipocontratoTable" style="font-size: 12px;" id="tipocontratoTable">
								...
								<tbody class="list" id="tipocontrato-table-body">
                           <tr>
	...
	<?php
		include "tipocontratoedit.php";
		include "tipocontratoadd.php";
		include APP_PATH.'/views/layouts/footer.php';
	?>
</div>
<script src="app/js/presupuesto/tipocontrato.js"></script>