...
class ConsignaModel {
   ...
   static public function getPorConsig() {
      ...
      $sql = "SELECT a.*, c.TerNombr, l.codigo, l.nombre AS banco_nombre, k.sucursal AS banco_sucursal, 
         ...
         WHERE a.id_empresa = :id_empresa AND clase = '1' AND a.capital_pagado = 0 AND (a.status = '1' OR a.status = 'D') AND COALESCE(
   ...
   static public function getConsignacion($data) {
      ...
      $sql = "SELECT a.id_empresa, a.id_consigna, a.consecutivo, a.id_cheque, a.BanCodig, d.BanNombr, a.fecha, 
         ...
         c.fecha AS fecha_cheque, c.valor_cheque, c.capital_pagado, c.comision, l.codigo AS banco_codigo, e.TerDocId, 
         f.TerNombr, 
         ...
         WHERE a.id_empresa = :idEmpresa AND a.consecutivo = :consecutivo
      ";
      $stmt = $connection->prepare($sql);
      $stmt->bindParam(":idEmpresa", $_SESSION["id_empresa"], PDO::PARAM_INT);
      $stmt->bindParam(":consecutivo", $data["idConsigna"], PDO::PARAM_INT);
      $stmt->execute();
      $response = $stmt->fetchAll(PDO::FETCH_ASSOC);
      $connection = null;
      $stmt = null;
