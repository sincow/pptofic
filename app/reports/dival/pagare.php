...
class imprimirDocumento {
	...
	public function traerImpresionDocumento() {
      ...
      if ($this->informe == null) {
			...
			$pdf->Text(115, 185, 'Registro no encontrado');
			$pdf->Rotate(0);
      ...
      $pdf->SetFillColor(43, 114, 171);
		...
		$pdf->SetTextColor(0, 0, 0);
		$pdf->Cell(185, 5, 'PAGARÉ', 0, 0, 'C', false);
      // $pdf->Ln(0);
		// $pdf->Cell(190, 5, $this->informe['numero'], 0, 0, 'R', false);
      $pdf->Ln(12);
		$pdf->SetFont('Arial', '', 9);
      $longDate = longDate($this->informe['fecha']);
      $pdf->Cell(185, 0, "LUGAR Y FECHA DE FIRMA: BARRANQUILLA, ".$longDate, 0, 0, 'L', false);
      $pdf->Ln(5);
      $pdf->Cell(185, 0, "PAGARÉ NÚMERO: ".$this->informe['numero'], 0, 0, 'L', false);
      $pdf->Ln(3);
      ...
      $x_axis = $pdf->getx();
      $pdf->MultiCell(185, 4, $cantidad, 0, 'L');
      $pdf->Ln(3);
      $pdf->Cell(185, 0, "INTERESES DURANTE EL PLAZO: MAXIMO LEGAL VIGENTE SUPERBANCARIA", 0, 0, 'L', false);
      $pdf->Ln(5);
      $pdf->Cell(185, 0, "INTERESES DE MORA: MAXIMO LEGAL VIGENTE SUPERBANCARIA", 0, 0, 'L', false);
      $pdf->Ln(3);
      $pdf->MultiCell(185, 4, "PERSONA A QUIEN DEBE HACERSE EL PAGO: RICARDO RAMON OLIVA MEDINA Y/O ".$_SESSION['companyname'], 0, 'L');
      $pdf->Ln(3);
      $longDate = longDate($this->informe['vencimiento']);
		$pdf->SetFont('Arial', 'B', 9);
      $pdf->Cell(185, 0, "FECHA DE VENCIMIENTO DE LA OBLIGACION: ".$longDate, 0, 0, 'L', false);
		$pdf->SetFont('Arial', '', 9);
      $pdf->Ln(5);
      $pdf->Cell(185, 0, "DEUDORES:", 0, 0, 'L', false);
      $pdf->Ln(3);
      $pdf->MultiCell(185, 4, "Nombre e identificación: ".$this->informe['TerNombr']." Con ".$this->informe['TerTiDoc']." Nro ".$this->informe['TerDocId'], 0,'L');
      $pdf->Ln(0);
      $pdf->MultiCell(185, 4, "Nombre e identificación: ".$this->informe['TerNombr2']." Con ".$this->informe['TerTiDoc2']." Nro ".$this->informe['TerDocId2'], 0, 'L');
      $pdf->Ln(0);
      if ($this->informe['TerDocId3'] != null && $this->informe['TerNombr3'] != null) {
         $pdf->MultiCell(185, 4, "Nombre e identificación: ".$this->informe['TerNombr3']." Con ".$this->informe['TerTiDoc3']." Nro ".$this->informe['TerDocId3'], 0, 'L');
         $pdf->Ln(0);
      }
      if ($this->informe['TerDocId4'] != null && $this->informe['TerNombr4'] != null) {
         $pdf->MultiCell(185, 4, "Nombre e identificación: ".$this->informe['TerNombr4']." Con ".$this->informe['TerTiDoc4']." Nro ".$this->informe['TerDocId4'], 0, 'L');
         $pdf->Ln(0);
      }
      $pdf->Ln(1);
      $pdf->SetFont('Arial', 'B', 9);
      ...
      $pdf->Ln(7);
      $longDate = longDate($this->informe['fecha']);
      $pdf->Cell(185, 0, "En constancia de lo anterior, se suscribe este documento el dia ".$longDate, 0, 0, 'L', false);
      $pdf->Ln(7);
      $pdf->Cell(185, 0, "OTORGANTES:", 0, 0, 'L', false);
      $pdf->Ln(7);
      $pdf->SetDrawColor(244, 244, 244);
      $pdf->SetFillColor(244, 244, 244);
		$pdf->Cell(90, 26, "", 1, 0, 'L', true);
      $pdf->Cell(5);
		$pdf->Cell(90, 26, "", 1, 0, 'L', true);
      $pdf->Ln(2);
		$pdf->SetFont('Arial', '', 8);
      ...
      $pdf->Ln(16);
		...
		$pdf->SetFont('Arial', '', 7);
      $pdf->Cell(90, 0, $this->informe['TerNombr'], 0,'L');
      $pdf->Cell(5);
      $pdf->Cell(90, 0, $this->informe['TerNombr2'], 0,'L');
		$pdf->SetFont('Arial', '', 8);
      $pdf->Ln(0);
		$pdf->SetTextColor(0, 0, 0);
      ...
      $pdf->Ln(16);
		$pdf->SetTextColor(210, 210, 210);
		$pdf->SetFont('Arial', '', 7);
      $pdf->Cell(90, 0, $this->informe['TerNombr3'], 0,'L');
      $pdf->Cell(5);
      $pdf->Cell(90, 0, $this->informe['TerNombr4'], 0,'L');
		$pdf->SetFont('Arial', '', 8);
      $pdf->Ln(0);
		$pdf->SetTextColor(0, 0, 0);
		$pdf->Cell(90, 0, '________________________________________________________', 0, 0, 'L', false);
