...
document.addEventListener("DOMContentLoaded", function() {
   ...
   document.getElementById('porcentaje_comision').addEventListener('input', function() {
      ...
      if (value.includes('.') && value.split('.')[1].length > 6) {
         const entera = value.split('.')[0].substring(0, 2);
         const decimal = value.split('.')[1].substring(0, 6);
         this.value = entera + '.' + decimal;
      }
   });
   ...
   .then(saldos => {
      if (saldos.length === 0) {
         ...
         saldos.forEach(saldo => {
            ...
            interxCobrar += parseFloat(saldo.intereses_cobrado);
            SaldosHtml += `
               <tr>
                  ...
                  <td class="fw-semi-bold py-2 text-end">${parseFloat(saldo.intereses_cobrado).toLocaleString('es-MX', { minimumFractionDigits: 0, maximumFractionDigits: 0 })}</td>
