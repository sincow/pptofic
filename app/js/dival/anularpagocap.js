document.addEventListener("DOMContentLoaded", function() {
   ...
   if (numDocAnu) {
      ...
      numDocAnu.addEventListener('change', () => {
         ...
         formData.append("action", "getPagoCapById");
   ...
   if (btnAnularDocum) {
      document.getElementById('btnAnularDocum').addEventListener('click', () => {
         swal.fire({
            title: '¿Está seguro de anular el Pago Capital?',
...
function getDocumentDetails(data) {
   ...
   document.getElementById('id_pago').value = data['pagoCapital'].id_pago;
   document.getElementById('id_cheque').value = data['cheque'].id_cheque;
   document.getElementById('valor').value = data['pagoCapital'].valor;
   ...
   const innerHtmlDoc = `
      ...
      <div class="row pt-2 border-top">
         ...
         <div class="col-4 col-lg-3 p-0 fs--1 text-end"><strong>${formatCurrency(parseFloat(data['pagoCapital'].valor),0)}</strong></div>
      </div>
      <div class="row">
         <div class="col-5 text-label fs--1">Fecha del Pago:</div>
         <div class="col-4 col-lg-3 p-0 fs--1 text-end">${data['pagoCapital'].fecha}</div>
      </div>
   `;
...
function anularDocum(paramsList) {
   ...
   formData.append("modulo", "dival");
   formData.append("option", "cheques");
   formData.append("action", "anularPagoCap");
