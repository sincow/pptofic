...
document.addEventListener("DOMContentLoaded", function() {
   ...
   //*********************************************************************************************
   document.getElementById('interes_pagar').addEventListener('change', function() {
      const valMax = $(this).attr('valMax');
      if (parseFloat(this.value) > parseFloat(valMax)) {
         this.value = valMax;
      }
   });
   document.getElementById('interes_pagar').addEventListener('input', function() {
      this.value = this.value.replace(/[^\d]/g, '');
   });
   document.getElementById('interes_pagar').addEventListener('blur', function() {
      if (this.value) {
         const number = parseInt(this.value);
         this.value = number.toLocaleString('es-MX', { minimumFractionDigits: 0, maximumFractionDigits: 0 });
      }
   });
   document.getElementById('interes_pagar').addEventListener('focus', function() {
      this.value = this.value.replace(/\,/g, '');
   });


   //*********************************************************************************************
   document.getElementById('pagointeresForm').addEventListener('submit', function(e) {
      e.preventDefault();
      e.stopPropagation();
      ...
      let isValid = "";
      let isRun = true;
      isValid = document.getElementById('interes_pagar').value != 0.00;
      document.getElementById('interes_pagar').classList.remove('is-invalid');
      document.getElementById('interes_pagar').classList.remove('is-valid');
      document.getElementById('interes_pagar').classList.add(isValid ? 'is-valid' : 'is-invalid');
      document.getElementById('interes_pagar').classList.contains('is-invalid') ? document.getElementById('interes_pagar').focus() : "";
      !isValid ? isRun = false : "";
...
function queryDocument(idDccument) {
   ...
   .then(response => {
      if (response != false) {
         ...
         document.getElementById('interes_pagar').value = parseFloat(0.00).toFixed(2);
         document.getElementById('interes_pagar').setAttribute('valMax', Number(IntPendiente));
...
function createAcounting(paramsList) {
   ...
   if (Compte != "") {
      let AsiDescr = "Pago de Intereses del documento Nro " + document.getElementById('numero').value;
      ...
      acountingList.push({
         ...
         "AsiDescr": AsiDescr,
         "AsiNatur": "D",
         "AsiValor": parseFloat(document.getElementById('interes_pagar').value.replace(/,/g, '')).toFixed(2),
         "AsiVBase": 0
      })
      const ingInt = paramsList.find(param => param.ParCodig === "CU5").ParValor;
      acountingList.push({
         "CueCodig": ingInt,
         "AsiDescr": AsiDescr,
         "AsiNatur": "C",
