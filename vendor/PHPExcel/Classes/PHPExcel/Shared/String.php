...
//		$c0 = ord($str{0});    ALVARO CARMONA
//		$c1 = ord($str{1});    ALVARO CARMONA
		...
		$newstr = '';
		for($i=0;$i<$len;$i+=2) {
//			if( $bom_be ) { $val = ord($str{$i})   << 4; $val += ord($str{$i+1}); }  ALVARO CARMONA
			if( $bom_be ) { $val = ord(substr($str,$i,1))   << 4; $val += ord(substr($str,$i+1,1)); }
//			else {        $val = ord($str{$i+1}) << 4; $val += ord($str{$i}); }   ALVARO CARMONA
			else {        $val = ord(substr($str,$i+1,1)) << 4; $val += ord(substr($str,$i,1)); }
			$newstr .= ($val == 0x228) ? "\n" : chr($val);
		}
		return $newstr;
