...
class PHPExcel_Calculation {
	...
	public static function _unwrapResult($value) {
		if (is_string($value)) {
//			if ((isset(substr$value{0})) && ($value{0} == '"') && (substr($value,-1) == '"')) {   ALVARO CARMONA
			if ((isset($value)) && (substr($value,0,1) == '"') && (substr($value,-1) == '"')) {
...
//		if ((!isset($formula{0})) || ($formula{0} != '=')) return array();  ALVARO CARMONA
		if ((!isset($formula)) || (substr($formula,0,1) != '=')) return array();
		$formula = ltrim(substr($formula,1));
//		if (!isset($formula{0})) return array();    ALVARO CARMONA
		if (!isset($formula)) return array();
	...
	public function _calculateFormulaValue($formula, $cellID=null, PHPExcel_Cell $pCell = null) {
		...
		$formula = trim($formula);
//		if ($formula{0} != '=') return self::_wrapResult($formula);  ALVARO CARMONA
		if (substr($formula,0,1) != '=') return self::_wrapResult($formula);
		$formula = ltrim(substr($formula, 1));
//		if (!isset($formula{0})) return self::_wrapResult($formula);  ALVARO CARMONA
		if (!isset($formula)) return self::_wrapResult($formula);
...
//		if (($wsTitle{0} !== "\x00") && ($this->_cyclicReferenceStack->onStack($wsCellReference))) {  ALVARO CARMONA
		if (($wsTitle !== "\x00") && ($this->_cyclicReferenceStack->onStack($wsCellReference))) {
...
//echo 'Assessing Expression '.substr($formula, $index),PHP_EOL;
			$opCharacter = substr($formula,$index,1);	//	Get the first character of the value at the current index position
//echo 'Initial character of expression block is '.$opCharacter,PHP_EOL;
			if ((isset(self::$_comparisonOperators[$opCharacter])) && (strlen($formula) > $index) && (isset(self::$_comparisonOperators[substr($formula,$index+1,1)]))) {
//				$opCharacter .= $formula{++$index};  ALVARO CARMONA
				$opCharacter .= substr($formula,++$index,1);
...
//						echo 'Element '.$localeConstant.' is an Excel Constant<br />';
			...
			//	Ignore white space
//			while (($formula{$index} == "\n") || ($formula{$index} == "\r")) {  ALVARO CARMONA
			while ((substr($formula,$index,1) == "\n") || (substr($formula,$index,1) == "\r")) {
				++$index;
			}
//			if ($formula{$index} == ' ') {  ALVARO CARMONA
			if (substr($formula,$index,1) == ' ') {
//				while ($formula{$index} == ' ') {   ALVARO CARMONA
				while (substr($formula,$index,1) == ' ') {
					++$index;
				}
...
//				} elseif ((is_numeric($token)) || ($token === NULL) || (is_bool($token)) || ($token == '') || ($token{0} == '"') || ($token{0} == '#')) {  ALVARO CARMONA
					} elseif ((is_numeric($token)) || ($token === NULL) || (is_bool($token)) || ($token == '') || (substr($token,0,1) == '"') || (substr($token,0,1) == '#')) {
...
//			if ($operand > '' && $operand{0} == '"') { $operand = self::_unwrapResult($operand); }  ALVARO CARMONA
			if ($operand > '' && substr($operand,0,1) == '"') { $operand = self::_unwrapResult($operand); }
			//	If the string is a numeric value, we treat it as a numeric, so no further testing
			if (!is_numeric($operand)) {
				//	If not a numeric, test to see if the value is an Excel error, and so can't be used in normal binary operations
//				if ($operand > '' && $operand{0} == '#') {  ALVARO CARMONA
				if ($operand > '' && substr($operand,0,1) == '#') {
...
//		if (is_string($operand1) && $operand1 > '' && $operand1{0} == '"') { $operand1 = self::_unwrapResult($operand1); } ALVAO CArMONA
		if (is_string($operand1) && $operand1 > '' && substr($operand1,0,1) == '"') { $operand1 = self::_unwrapResult($operand1); }
//		if (is_string($operand2) && $operand2 > '' && $operand2{0} == '"') { $operand2 = self::_unwrapResult($operand2); }
		if (is_string($operand2) && $operand2 > '' && substr($operand2,0,1) == '"') { $operand2 = self::_unwrapResult($operand2); }
